...
#if defined(__IPHONEOS__) || defined(TARGET_OS_IPHONE)
#include "platform/ios/ios_bridge.h"
#endif
...
#if defined(__IPHONEOS__) || defined(TARGET_OS_IPHONE)
// Exported so the iOS on-screen touch-controls overlay (Swift) can feed
// button presses into the same path as keyboard/gamepad input, without
...
#if defined(__IPHONEOS__) || defined(TARGET_OS_IPHONE)
// On iOS there is no meaningful "search upward from cwd" step: sandboxing
// means the only writable/readable location we control is the app's
// Documents directory. ios_bridge.m already chdir()'d the process into
// Documents before SDL_main/main() runs, so this becomes a no-op. Kept as
// a function so call sites elsewhere in main.c don't need to change.
static void SwitchDirectory() {
  // Intentionally empty on iOS — see platform/ios/ios_bridge.m.
}
#else
