...
uint8 *ApplyBps(const uint8 *src, size_t src_size_in,
  ...
  if (memcmp(bps, "BPS1", 4))
    return NULL;
  // armv7: bps_end is at an arbitrary byte offset in the file buffer, so these
  // footer words are read with memcpy (an unaligned uint32* deref may be merged
  // into LDM/LDRD by the compiler, which faults on unaligned addresses).
  uint32 crc_src, crc_dst, crc_patch;
  memcpy(&crc_src, bps_end, 4);
  memcpy(&crc_dst, bps_end + 4, 4);
  memcpy(&crc_patch, bps_end + 8, 4);
  if (crc32(src, src_size_in) != crc_src)
    return NULL;
  if (crc32(bps, bps_size - 4) != crc_patch)
    return NULL;
