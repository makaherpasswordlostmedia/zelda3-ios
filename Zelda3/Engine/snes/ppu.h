...
struct Ppu {
  ...
  // armv7 (iOS): ClearBackdrop() in ppu.c stores uint64 into these, which the
  // compiler may emit as STRD. STRD requires 4-byte (LDRD/STRD: 8-byte on some
  // cores) alignment even where plain LDR/STR are allowed unaligned, and the
  // struct layout differs between 32/64-bit because of the pointer field above.
  // Force 8-byte alignment so this holds regardless of preceding fields.
  PpuPixelPrioBufs bgBuffers[2] __attribute__((aligned(8)));
  PpuPixelPrioBufs objBuffer __attribute__((aligned(8)));
